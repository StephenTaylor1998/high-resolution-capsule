...
if __name__ == '__main__':
    # sq = PrimaryCaps(4, 4, 5, 3, 2, groups=2, init_mode='he_normal')
    # sq = PrimaryCaps(4, 4, 5, 3, 2, groups=2, init_mode='glorot')
    sq = DigitCaps(15, 15, 4, 8,
                   10, 16, routing=3, init_mode='glorot')
    length = Length()
    # inp = torch.ones((1, 4, 8, 224, 224))
    inp = torch.ones((1, 15, 15, 4, 8))
    out = sq(inp)
    # print(out)
    print(out.shape)
    out = length(out)
